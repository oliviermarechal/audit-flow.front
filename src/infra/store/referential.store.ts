...
import type {
	...
	ReferentialStoreInterface,
	ReferentialVersionInterface
} from '../../domain';
...
class ReferentialStore implements ReferentialStoreInterface {
	...
	async updateReferential(referential: ReferentialInterface): Promise<void> {
		await this.referentialWritable.update((referentials) => {
			return [...referentials.filter((r) => r.id !== referential.id), referential];
		});
	}
    ...
    async removeVersion(referentialId: string, versionId: string): Promise<void> {
        this.referentialWritable.update((referentials) => {
            const referentialToRemoveVersion = referentials.find(r => r.id === referentialId);
            if (!referentialToRemoveVersion || !referentialToRemoveVersion.versions) {
                return referentials;
            }

            referentialToRemoveVersion.versions = [
                ...referentialToRemoveVersion.versions.filter(v => v.id !== versionId)
            ];

            return [
                ...referentials.filter(r => r.id !== referentialId),
                referentialToRemoveVersion
            ]
        })
    }
