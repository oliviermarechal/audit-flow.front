...
export async function createReferential(
	referential: ReferentialInterface
): Promise<Referential | void> {
	const httpClient = HttpClientSingleton.getInstance();
	const response = await httpClient.post('/referentials', { ...referential });
...
export async function updateReferential(
	...
	const httpClient = HttpClientSingleton.getInstance();
	const response = await httpClient.put(`/referentials/${referential.id}`, { ...referential });
