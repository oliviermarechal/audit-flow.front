...
export class ReferentialVersion implements ReferentialVersionInterface {
	...
	public getPayload() {
		return {
			...
			dataMapping:
				this.syncMode === ReferentialSyncModeEnum.API
					? {
	...
	humanizeStatus(): string | undefined {
		switch (this.status) {
			case ReferentialVersionStatusEnum.Draft:
				return 'Brouillon';
				break;
			case ReferentialVersionStatusEnum.Published:
				return 'Publié';
				break;
			case ReferentialVersionStatusEnum.Archived:
				return 'Archivé';
				break;
		}
	}
